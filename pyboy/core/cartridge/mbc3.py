    ...
    def setitem(self, address, value):
        if 0x0000 <= address < 0x2000:
            if (value & 0b00001111) == 0b1010:
                ...
                # disables RAM."
                self.rambank_enabled = False
                logger.debug(b"Unexpected command for MBC3: Address: 0x%0.4x, Value: 0x%0.2x", address, value)
            ...
            if self.rtc_enabled:
                ...
                logger.debug(b"RTC not present. Game tried to issue RTC command: 0x%0.4x, 0x%0.2x", address, value)
            ...
            if self.rambank_enabled:
                if self.rambank_selected <= 0x03:
                    ...
                    logger.error(b"Invalid RAM bank selected: 0x%0.2x", self.rambank_selected)
        else:
            logger.error(b"Invalid writing address: 0x%0.4x", address)
