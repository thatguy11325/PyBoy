...
class RTC:
    def __init__(self, filename):
        ...
        if not os.path.exists(self.filename):
            logger.info(b"No RTC file found. Skipping.")
        else:
    ...
    def writecommand(self, value):
        if value == 0x00:
            ...
            logger.warning(b"Invalid RTC command: %0.2x", value)

    def getregister(self, register):
        if not self.latch_enabled:
            logger.debug(b"RTC: Get register, but nothing is latched! 0x%0.2x", register)
        ...
        if register == 0x08:
            ...
            logger.warning(b"Invalid RTC register: %0.4x", register)

    def setregister(self, register, value):
        if not self.latch_enabled:
            logger.debug(b"RTC: Set register, but nothing is latched! 0x%0.4x, 0x%0.2x", register, value)
        ...
        if register == 0x08:
            ...
            if self.halt == 0:
                ...
                logger.warning(b"Stopping RTC is not implemented!")

            self.timezero -= int(t / 3600 / 24) - (day_high << 8)
            self.day_carry = day_carry
        else:
            logger.warning(b"Invalid RTC register: %0.4x %0.2x", register, value)
